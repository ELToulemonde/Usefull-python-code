...
__all__ = ["fast_is_constant", "fast_is_double", "fast_is_bijection", "filter_cdb"]
    ...
    if n > 1:
        ...
        for i in range(1, max_power + 1):
            ...
            if obj.dtype.type in [np.str_, np.object_, np.string_]:
                ...
                n_nans = np.sum(np.isnan(obj[I]))
                if n_nans == len(I):
                    # All Nans
                    if not np.any(np.isnan(unique_list)):
...
def filter_cdb(data_set: pd.DataFrame, type="cdb", verbose=True):
    if "c" in type:
        if verbose:
            print("filter_cdb: Start to look for constant.")
        constant_list = []
        for col in tqdm(data_set.columns):
            if fast_is_constant(data_set[col]):
                constant_list.append(col)
                if verbose:
                    print("filter_cdb: " + col + " is constant. Will drop it.")
        if verbose:
            print("filter_cdb: found " + str(len(constant_list)) + " constant dropping them.")
        data_set.drop(columns=constant_list, inplace=True)
    if "d" in type:
        if verbose:
            print("filter_cdb: Start to look for double.")
        double_list = []
        ...
        for col_ind1 in tqdm(range(0, len(col_names) - 1)):
            ...
            for col_ind2 in range(col_ind1 + 1, len(col_names)):
                ...
                if fast_is_double(data_set[col1], data_set[col2]):
                    ...
                    if verbose:
                        print("filter_cdb: " + col1 + " is in double of " + col2 + ". Will drop it.")
                    break
        if verbose:
            print("filter_cdb: found " + str(len(double_list)) + " double dropping them.")
        data_set.drop(columns=double_list, inplace=True)
    if "b" in type:
        if verbose:
            print("filter_cdb: Start to look for bijection.")
        bijection_list = []
        col_names = data_set.columns
        for col_ind1 in tqdm(range(0, len(col_names) - 1)):
            col1 = col_names[col_ind1]
            for col_ind2 in range(col_ind1 + 1, len(col_names)):
                ...
                if fast_is_bijection(data_set[col1], data_set[col2]):
                    ...
                    if verbose:
                        print("filter_cdb: " + col1 + " is a bijection of " + col2 + ". Will drop it.")
                    break
        if verbose:
            print("filter_cdb: found " + str(len(bijection_list)) + " bijection dropping them.")
